...
class DoneReceiptTabBarView extends StatefulWidget {
  @override
  _DoneReceiptTabBarViewState createState() => _DoneReceiptTabBarViewState();
}

class _DoneReceiptTabBarViewState extends State<DoneReceiptTabBarView>
    with AutomaticKeepAliveClientMixin<DoneReceiptTabBarView> {
  final _userBloc = UserBloc();
  final _receiptBloc = ReceiptBloc();

  bool isLoading = true;
  List<Receipt> _lstSortDoneReceipts = [];
  ...
  @override
  void initState() {
    _receiptBloc.getReceiptDone(_userBloc.userInfo.userID).then((bool a) {
      setState(() {
        isLoading = false;
      });
    });
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        initialData: _receiptBloc.doneReceipts,
        stream: _receiptBloc.observableReceiptDone,
        builder: (context, snapshot) {
          if ((snapshot.data == null || snapshot.data.length == 0) &&
            ...
            _lstSortDoneReceipts=sortByDate(_receipts);
            return RefreshIndicator(
              child: ListView.builder(
                  itemCount: _lstSortDoneReceipts.length,
